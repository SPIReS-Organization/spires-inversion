...
// The big arrays — imagery spectra (spectrum_background / spectrum_target) and
// the LUT (lut_reflectances) — are stored as float32 to halve memory; every
// value is promoted to double at read time inside the kernel, so the math and
// NLopt run in full double precision. Coordinate axes, shade, and the parameter
// vector stay double. float32 is a property of the storage signature, not a
// name suffix.
